...
module.exports = {
    Isim: "sicil",
    Komut: ["sicil"],
    Kullanim: "sicil temizle ID / sicil <@wonxen/ID>",
  ...
  let embed = new MessageEmbed().setColor("RANDOM").setAuthor(wonxen.Tag , message.guild.iconURL({dynamic: true, size: 2048})).setFooter(wonxez.Footer,message.guild.iconURL({ dynamic: true }))
  ...
  if(!uye2) return message.channel.send(`Hata: Lütfen bir üye Id giriniz!  __Örn:__  \`${client.sistem.Prefix}sicil temizle <ID>\``).then(x => x.delete({timeout: 5000}));
  ...
  sicil = sicil.reverse();
  let sicilpaneli = sicil.length > 0 ? sicil.map((value, index) => `[\`#${value.No}\` - \`${value.Tip}\`] ${message.guild.members.cache.has(value.Yetkili) ? message.guild.members.cache.get(value.Yetkili) : value.Yetkili} tarafından **${value.Sebep}** sebebiyle **${new Date(value.Zaman).TurkiyeTarihDuzeni()}** tarihinde cezalandırıldı.`).join("\n") : `Üyenin herhangi bir sicil kayıtı bulunamadı.\n`;
  client.embedGenislet(`${uye} Adlı kullanıcının veri tabanında kayıtlı sicili;\n\n${sicilpaneli}`,
                           {name: `${message.author.tag}`, icon: message.author.avatarURL({dynamic: true})},
                           {name: `Kullanıcının sicil güvenliği: ${sicilguvenli}`, icon: message.guild.iconURL({ dynamic: true })},
