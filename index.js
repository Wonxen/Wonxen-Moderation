...
client.veri = require("./src/./Settings/Config.json");
client.sistem = require("./src/./Settings/System.json");
client.bots = require("./src/./Settings/Bot.json");
client.modüller = {};
...
fs.readdir("./src/Commands", (err, files) => {
  ...
  console.log(`[ ${files.length} ] Adet moderasyon komutları yüklenecek.`);
  files.forEach(file => {
    let referans = require(`./src/Commands/${file}`);
...
fs.readdir("./src/Events/", (err, files) => {
  ...
  files.forEach(fileName => {
    let referans = require("./src/Events/" + fileName);
    referans.onLoad(client);
    client.on(referans.Etkinlik, referans.onRequest);
    console.log(
      `[ ~ EVENT ~ ] (${fileName}) isimli event yüklendi.`
    );
  });
});
//--------------------------------------------------------------------------------------------------------------\\
...
client.on("message", (message) => {
  if ([".tag", "!tag", "tag"].includes(message.content.toLowerCase())) {
    return message.lineReply(`\`${message.client.veri.Tag}\``);
  }
  if (message.author.bot || !message.content.startsWith(client.sistem.Prefix) || !message.channel || message.channel.type == "dm") return;
  let args = message.content
    .substring(client.sistem.Prefix.length)
    .split(" ");
...
client.on("guildMemberAdd", async member => {
  ...
  if (wonxenkalkmazban && wonxenkalkmazban.some(id => `k${member.user.id}` === id)) return;
  let wonxen = client.veri;
  let wonxenveri = db.get("ayar") || {};
  ...
  if (wonxenveri.YasakTaglar && !wonxenveri.YasakTaglar.some(tag => member.user.username.includes(tag)) && yasakTaglilar.some(x => x.includes(member.id))) await cezaDb.set('yasakTaglilar', yasakTaglilar.filter(x => !x.includes(member.id)));
  if (wonxenveri.YasakTaglar && wonxenveri.YasakTaglar.some(tag => member.user.username.includes(tag))) {
    if (wonxen.Roller.YasakliTagRole) member.roles.set([wonxen.Roller.YasakliTagRole]).catch();
    if (!yasakTaglilar.some(id => id.includes(member.id))) cezaDb.push('yasakTaglilar', `y${member.id}`);
    member.send(`**${wonxen.Tag} ** Yasak tag'da bulunduğunuz için otomatik olarak cezalıya atıldınız tagı çıkartana kadar cezalıda kalmaya devam ediceksin.`).catch();
  } else if (guvenili) {
    if (wonxen.Roller.SupheliRole) member.roles.set([wonxen.Roller.SupheliRole]).catch();
    if (wonxen.Kanallar.supheliLogKanali && member.guild.channels.cache.has(wonxen.Kanallar.supheliLogKanali)) return;
  } else if (wonxen.RegisterSystem.Unregistered) member.roles.add(wonxen.RegisterSystem.Unregistered).catch();
  if (sürelisusturma.some(x => x.id === member.id) || kalicisusturulma.some(x => x.includes(member.id))) member.roles.add(wonxen.Roller.MuteRole).catch();
  if (sessusturulma.some(x => x.id === member.id) && member.voice.channel) member.voice.setMute(true).catch();
  if (wonxen.Tagsız) member.setNickname(`${wonxen.Tagsız} İsim | Yaş`).catch();
  else if (wonxen.Tag) member.setNickname(`${wonxen.Tag} İsim | Yaş`).catch();
  ...
  wonxenhook.send(`
  Sunucumuza hoş geldin, ${member} - (\`${member.id}\`) Seninle beraber toplam **${member.guild.memberCount}** kişiyiz.\n
  Hesabın **${tarih}** tarihinde **${client.gecmisTarihHesaplama(member.user.createdAt)}** oluşturulmuş.\n
  Tagımızı (${Config.Tag}) alarak bize destek olabilirsin! <#${Config.WelcomeSystem.Rules}> kanalındaki kurallarımızı okuduğun varsayılarak kaydedileceksin.\n
  Kayıt olmak için <#${Config.WelcomeSystem.Welcome}> kanalında bekleyebilirsin. <@&${Config.RegisterSystem.Register[0]}>Rolünde bulunan yetkililerimiz sizinle ilgilenecektir.\n
  `);
})
...
//-----------------------------------------------------------------------------------------------------------------------\\
//-----------------------------------------------------------------------------------------------------------------------\\
client.login(client.sistem.Token).catch(err => console.error("[~ Wonxen ~] Discord API Botun Tokenini doğrulayamadı."));