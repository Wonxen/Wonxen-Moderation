...
module.exports = {
    ...
    Kullanim: "sustur <@wonxen/ID> <süre> <sebep>",
  ...
  onRequest: async function (client, message, args, guild) {
    ...
    if(!uye) return message.channel.send(`Hata: Lütfen bir üye etiketleyin veya Id giriniz!  __Örn:__  \`${client.sistem.Prefix}mute <@wonxen/ID> <1s/1m/1h/1d> <sebep>\``).then(sil => sil.delete({timeout: 5000}));
    ...
    if(!sure || !ms(sure) || !reason) return message.channel.send(`Hata: Lütfen bir süre belirleyin!  __Örn:__  \`${client.sistem.Prefix}jail <@wonxen/ID> <1s/1m/1h/1d> <sebep>\``).then(sil => sil.delete({timeout: 5000}));
    ...
    message.lineReply(`${muteicon} ${uye} adlı kullanıcı, ${message.author} tarafından, \`${reason}\` nedeni ile \`${mutezaman}\` süresince  metin kanallarında susturuldu! \`(Ceza ID: #${cezano})\``).then(message.react(wonxenveri.Others.CheckTick));
    logs.wsend(embed.setAuthor(uye.user.tag, uye.user.avatarURL({ dynamic: true })).setThumbnail(uye.user.avatarURL({ dynamic: true })).setDescription(`${uye} adlı kişi sunucumuzda **Metin kanallarından susturuldu.**\n\n\`•\` Ceza Numarası: \`${cezano}\`\n\`•\` Susturulma Sebebi: \`${reason}\`\n\`•\` Susturulma Saati:\`${client.tarihsel}\`\n\`•\` Susturulma Süresi: \`${mutezaman}\``).setFooter(wonxez.Footer,message.guild.iconURL({ dynamic: true }))).catch();
     }
};
