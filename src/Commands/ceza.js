...
module.exports = {
  ...
  let embed = new MessageEmbed().setColor("RANDOM").setAuthor(message.guild.name, message.guild.iconURL({dynamic: true, size: 2048}))
  ...
  if(sorgu == "bilgi" || sorgu == "info") {
    ...
    if(Number(args[1]) && args[1].length < 15) {
        ...
        let kisibilgi;
        if(kisi != `\`Bulunamayan Üye\`` && kisi.username) kisibilgi = `${kisi} (\`${kisi.tag}\` - \`${kisi.id}\`)`;
        if(!kisibilgi) kisibilgi = "Bilinmiyor"
        let kisi2 = await client.users.fetch(ceza.Yetkili).catch();
        let kisibilgi2;
        if(kisi2 != `\`Bulunamayan Üye\`` && kisi2.username) kisibilgi2 = `${kisi2} (\`${kisi2.tag}\` - \`${kisi2.id}\`)`;
       ...
       let cezabas = moment(Zaman).format("DD") + " " + aylar[moment(Zaman).format("MM")] + " " + moment(Zaman).format("YYYY HH:mm:ss") 
        ...
        \`➜\` Ceza Numarası: \`#${ceza.No}\`
        \`➜\` Ceza Tarihi: \`${cezabas}\`
        \`➜\` Ceza Süresi: \`${cezaZaman}\`
        \`➜\` Ceza Durumu: \`${cezabitme}\`
        `).addField(`Ceza Türü`, `__` + ceza.Tur + `__` , true).addField(`Ceza Sebebi`,`__` + ceza.Sebep + `__` ,true).setFooter(wonxez.Footer,message.guild.iconURL({ dynamic: true })))
    }
  return 
  };  
  ...
  let cn = args[1]
  if(!cn) return message.channel.send(`Hata: Lütfen bir üye Id giriniz!  __Örn:__  \`${client.sistem.Prefix}ceza temizle <#Ceza-No>\``).then(x => x.delete({timeout: 5000}));
  let ceza = await kDb.fetch(`ceza.${args[1]}`)
  if(!ceza) {
...
if(sorgu == "sıfırla") {
  ...
  message.channel.send(`Bu sunucunun (\`${client.sistem.ID}\`) tüm üyelerin ceza ve sunucu ceza numaralarının bilgileri komple temizlendi!`).then(x => x.delete({timeout: 5000})); 
  message.react(Config.Others.CheckTick)
  return 
};
  message.channel.send(`Hata: Ceza sorgusu yapmak için öncelikle __Ceza Numarası__ ID'sine ihtiyacın var __Örn__: \`${client.sistem.Prefix}ceza bilgi <#Ceza-No>\``).then(sil => sil.delete({timeout: 5000}));
}
};